...
class Role(SerializationMixin, ContextMixin, BaseModel):
    ...
    def _process_role_extra(self):
        ...
        # 这一行调用了llm provider里面的create_llm_instance
        # logger.error('-'*100)
        # logger.error('in _process_role_extra')
        # logger.error('before _check_actions')
        self._check_actions()
        # logger.error('before _get_prefix')
        # 这一行调用了llm provider里面的create_llm_instance
        self.llm.system_prompt = self._get_prefix()
        # logger.error('before setting cost_manager')
        self.llm.cost_manager = self.context.cost_manager
        # logger.error('before self.rc.watch')
    ...
    def set_actions(self, actions: list[Union[Action, Type[Action]]]):
        ...
        """
        # logger.error('in setting action')
        # logger.error('len actions  '+str(len(actions)))
        self._reset()
    ...
    def _get_prefix(self):
        """Get the role prefix"""
        # logger.error('in getting prefix')
        if self.desc:
            return self.desc
        ...
        if self.rc.env and self.rc.env.desc:
            # logger.error('before self.rc.env.role_names')
            all_roles = self.rc.env.role_names()
            # logger.error('after self.rc.env.role_names')
            ...
            prefix += env_desc
        # logger.error('after getting prefix')
        return prefix
    ...
    async def _act(self) -> Message:
        ...
        # 这一步是具体在跑，self.rc.todo就是其任务，会调用action里面的act
        # print('before action')
        # 这里就写了repo，要删去
    ...
    async def _react(self) -> Message:
        ...
        Use llm to select actions in _think dynamically
        """
        # logger.error('in role._react')
        # logger.error(f"{self._setting}: to do {self.rc.todo}({self.rc.todo.name})")
        actions_taken = 0
        ...
        while actions_taken < self.rc.max_react_loop:
            ...
            if not todo:
                break
            # act
            # logger.error(f"{self._setting}: {self.rc.state=}, will do {self.rc.todo}")
            # logger.error(f'setting args for {self._setting}')
            self.rc.todo.set_args(args=self.args)
            rsp = await self._act()
            actions_taken += 1
    ...
    async def run(self, args, with_message=None) -> Message | None:
        """Observe, and think and act based on the results of the observation"""
        self.args = args
        # logger.error('in role.run')
        # logger.error(f"{self._setting}")
        if with_message:
            msg = None
