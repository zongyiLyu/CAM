...
class Context(BaseModel):
    ...
    def llm(self) -> BaseLLM:
        ...
        # logger.error('in context.llm, calling create_llm_instance')
        # logger.error(self.args)
        self._llm = create_llm_instance(self.config.llm,self.args)
    ...
    def llm_with_cost_manager_from_llm_config(self, llm_config: LLMConfig) -> BaseLLM:
        """Return a LLM instance, fixme: support cache"""
        # if self._llm is None:
        # logger.error('in context.llm_with_cost_manager_from_llm_config, calling create_llm_instance')
        # logger.error(self.args)
        llm = create_llm_instance(llm_config,self.args)
