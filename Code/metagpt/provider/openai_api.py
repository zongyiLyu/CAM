...
class OpenAILLM(BaseLLM):
    ...
    def re_init_client(self,model):
        self.pricing_plan = self.config.pricing_plan or self.model
        kwargs = self._make_client_kwargs(model)
        # logger.error('in openai api, setting model config')
        # logger.error(self.model)
        # logger.error(kwargs)
        self.aclient = AsyncOpenAI(**kwargs)

    def _make_client_kwargs(self,model='gpt') -> dict:
        # logger.error
        # self.model = 'gpt'
        ...
        if 'gpt' in model:
            key='YOUR_API_KEY'
            kwargs = {"api_key": key, "base_url": 'https://api.chatanywhere.tech/v1/'}
# os.environ["OPENAI_API_KEY"] = "sk-NsLLS6Bbm06SDgbx3BJkyHsEys50pj9TqlZB7PrIJHFSIzmI"
...
# os.environ["OPENAI_BASE_URL"] = "https://api.chatanywhere.tech/v1"
    ...
    def _cons_kwargs(self, messages: list[dict], timeout=USE_CONFIG_TIMEOUT, **extra_kwargs) -> dict:
        ...
        if 'qwen' in self.model:
            self.model = 'qwen/Qwen2.5-14B-Instruct'  # Replace with the actual model name for qwen
        kwargs = {
            "messages": messages,
