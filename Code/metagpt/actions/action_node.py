...
class ActionNode:
    ...
    async def _aask_v1(
        ...
        # logger.error(f"output_class_name: {output_class_name}")

        '''
        2025-10-21 22:03:19.048 | ERROR    | metagpt.actions.action_node:_aask_v1:441 - output_class_name: WritePRD_AN
        2025-10-21 22:03:23.874 | ERROR    | metagpt.actions.action_node:_aask_v1:441 - output_class_name: DesignAPI_AN
    ...
    def refresh_instruct_content(self,keys,changed_contents):
        output_class_name = f"{self.key}_AN"
        # logger.error(f'output class name:{output_class_name}')
        # lo
        mode = 'auto'
        ...
        for key, changed_content in zip(keys,changed_contents):
            parsed_data[key]=changed_content
        # parsed_data['Project Name']=''
        
            # logger.error(f'parsed_data after refresh :{parsed_data}')
        instruct_content = output_class(**parsed_data)
        self.instruct_content = instruct_content


    def refresh_instruct_content_prd(self,keys,changed_contents):
        output_class_name = f"{self.key}_AN"
        # logger.error(f'output class name:{output_class_name}')
        # lo
        mode = 'auto'
    ...
    def set_content(self, content):
        self.set_recursive("content", content)

    def set_instruct_content(self, instruct_content):
        self.set_recursive("instruct_content", instruct_content)
