...
from metagpt.monitor import monitor_code
...
class WriteCode(Action):
    ...
    need_monitor: bool = True


    @retry(wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
    async def write_code(self, prompt) -> str:
        # type(prompt) = 'str
        # logger.error(type(prompt))
    ...
    async def run(self, *args, **kwargs) -> CodingContext:
        # logger.error('calling writecode Node .run')
        ...
        if self.need_monitor and self.args['repair_code']==1:
            need_regenerate=False
            try:
                design_doc_dict=eval(coding_context.design_doc.content)
            except:
                logger.error('can not eval coding_context.design_doc.content')
                design_doc_dict={'Implementation approach':'','Data structures and interfaces':''}
            plan  = design_doc_dict['Implementation approach']
            requirement = requirement_doc.content
            # logger.error('in repairing code')
            # logger.error(code)
            # logger.error(plan)
            code_monitor_result,need_regenerate = await monitor_code(code=code,requirement=requirement, plan=plan,model=self.args['model'])
            # logger.error(code_monitor_result,need_regenerate)
            if need_regenerate:
                code = await self.write_code(prompt)
