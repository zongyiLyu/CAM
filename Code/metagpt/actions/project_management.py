...
from metagpt.actions.intervent import intervent_node_task
...
class WriteTasks(Action):
    ...
    async def _run_new_tasks(self, context):
        ...
        if self.args['do_intervent']==1 and 'task' in self.args['levels']:
            keys2change = []
            for i in range(len(self.args['levels'])):
                if self.args['levels'][i]=='task':
                    keys2change.append(self.args['keys'][i])
            node  = intervent_node_task(node,context,'task',keys2change,self.args)
        return node
